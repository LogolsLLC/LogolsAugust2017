...
namespace LogolsAlumni.DAL
{
    public class AlumniRepository : Repository
    {
        public int Insert(Alumni alumni)
        {
            ...
            {
                dbConnection.Open();
                return dbConnection.Query<InsertID>("Team1.InsertAlumni", new { FirstName = alumni.FirstName, LastName = alumni.LastName, Email = alumni.Email }, commandType: CommandType.StoredProcedure).First().ID;
            }
        }
        ...
        public IEnumerable<Alumni> GetAll()
        {
            ...
            {
                dbConnection.Open();
                return dbConnection.Query<Alumni>("Team1.GetAll", commandType: CommandType.StoredProcedure);
            }
        }
